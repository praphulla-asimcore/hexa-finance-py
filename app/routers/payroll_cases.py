...
async def gen_check(case_id: str, request: Request):
    ...
    airtable_list = None
    _debug = {}  # TODO(temp): remove once MISSING_BANK_ACCOUNT prod investigation is done
    if case_type != "PAYROLL":
        at = []
        try:
            at = await fetch_airtable_consultants()
            _debug["airtable_n"] = len(at)
        except Exception as e:
            _debug["airtable_err"] = str(e)[:200]
        try:
            airtable_list = build_consultant_list(db, at)
            _debug["merged_n"] = len(airtable_list)
        except Exception as e:
            _debug["merge_err"] = str(e)[:200]
            airtable_list = at or None
