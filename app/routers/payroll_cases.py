...
async def _create_or_update_statutory(kase: dict, db, triggered_by: str) -> None:
    ...
    # consultant_master/overrides don't carry these fields, so merging them in
    # would add nothing here while still risking an ambiguous-ID conflict
    # against stale Airtable rows (see build_consultant_list's docstring).
    try:
...
async def _upload_case_inner(
    ...
    # Fetch Airtable consultant list for statutory enrichment only (nationality /
    # contract type / EPF scheme) — non-blocking. Not merged with
    # consultant_master/overrides: this consumer doesn't need those (they don't
    # carry statutory fields), and merging would only add ambiguous-ID risk.
    airtable_list = None
    try:
        airtable_list = await fetch_airtable_consultants()
    except Exception:
...
async def gen_check(case_id: str, request: Request):
    ...
    # consultant_master/overrides/HexaFlow directory power the id-conflict /
    # consultant-match flags for CSI — NOT Airtable, which is deprecated (the
    ...
    # the match ambiguous and silently excluding someone with valid bank data
    # on file. Built here so a manually-uploaded case produces the same flags
    # it would have when the check was generated inline at upload time.
    ...
    # Notify arrangers when a CSI case has exceptions (moved here from upload,
    # since the check — and therefore the flag count — is now produced at this step).
...
async def reupload_case(
    ...
    airtable_list = None
    # Raw Airtable for statutory enrichment (nationality/contract type/EPF
    # scheme — consultant_master doesn't carry these fields). Bank-detail
    # matching below deliberately excludes Airtable: it's deprecated and can
    # carry stale/duplicate Employee IDs that collide with a clean
    # consultant_master record, making the match ambiguous and silently
    # excluding someone who actually has valid bank data on file.
    try:
