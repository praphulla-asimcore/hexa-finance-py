...
def send_invite(to: str, name: str, invite_url: str, role: str = "") -> None:
    ...
    _send(to, f"You've been invited to Hexa Finance — {role_label}", _wrap(f"""
        ...
        <p style="color:#555;font-size:14px;line-height:1.6;margin:0 0 16px">
          ...
          Click the button below to activate your account and sign in — no password needed.
          From then on, sign in any time at the login page with this same email address.
        </p>
        <a href="{invite_url}" style="display:inline-block;background:#6366f1;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px">Activate &amp; Sign In</a>
        <p style="color:#999;font-size:12px;margin-top:24px">This link expires in 48 hours and can be used once. If you did not expect this invitation, you can ignore this email.</p>
        <p style="color:#999;font-size:12px;margin-top:8px">© 2026 Hexamatics Nepal Private Limited</p>
    """))


def send_login_link(to: str, name: str, login_url: str) -> None:
    """One-time sign-in link emailed when a user requests one from /login."""
    _send(to, "Your Hexa Finance sign-in link", _wrap(f"""
        <h2 style="font-size:20px;font-weight:700;color:#111;margin:0 0 8px">Sign in to Hexa Finance</h2>
        <p style="color:#555;font-size:14px;line-height:1.6;margin:0 0 16px">
          Hi {name or to},<br/><br/>
        ...
        <p style="color:#999;font-size:12px;margin-top:24px">If you didn't request this, you can safely ignore this email — no one can sign in without it.</p>
    """))
